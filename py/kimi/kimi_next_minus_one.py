...
def kimi_next_minus_one(csv_path: Path = None) -> None:
    if csv_path is None:
        csv_path = (
    ...
    temp_path = write_temp_csv(Path(csv_path), all_rows[:-1], header, delimiter, suffix="_kimi_temp")

    old_stdout = sys.stdout
    sys.stdout = io.StringIO()
    predicted = kimi_next(temp_path, run_accuracy_test=False)
    ...
    print(f"\n[kimi_next] Main numbers accuracy: {correct}/{total} correct ({accuracy:.1f}%)")
