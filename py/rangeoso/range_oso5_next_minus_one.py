"""
range_oso5_next_minus_one.py — Accuracy test for range_oso5_next.
...
from range_oso5_next import range_oso5_next


def range_oso5_next_minus_one(csv_path: Path = None) -> None:
    if csv_path is None:
        csv_path = (
    ...
    temp_path = write_temp_csv(
        csv_path, all_rows[:-1], header, delimiter, suffix="_range_oso5_temp"
    )

    # Silently predict on the reduced dataset
    old_stdout = sys.stdout
    sys.stdout = io.StringIO()
    predicted = range_oso5_next(
        temp_path,
    ...
    print(f"\n[range_oso5_next] Main numbers accuracy: {correct}/{total} correct ({accuracy:.1f}%)")
...
if __name__ == "__main__":
    ...
    range_oso5_next_minus_one(_csv)
