"""
range_oso5_next.py — Range-Constrained OSO Order-5 Predictor
...
Two-stage prediction:
  ...
  1. RANGE STAGE — predict the [min, max] span of the next draw's five main
     ...
     range_oso3_next / range_oso4_next — see range_common.py.)
  ...
  2. OSO_ORDER5 STAGE — for each main column, predict candidates the same
     way oso_next's "order5 fallback" stage does (pattern_fallback(rows, 5)):
     what value followed the current 5-row window for that column,
     historically. Candidates are then ranked (5-row match frequency first,
  ...
  3. RANGE CONSTRAINT — the final pick for each column must fall inside the
     predicted [range_min, range_max]. If the raw order5 pick is in range,
...
oso_next predicts mega: order5-style pattern match on column 7, falling
...
# ---------------------------------------------------------------------------
# OSO order5-stage helpers
# ---------------------------------------------------------------------------

def _order5_tail_frequencies(rows: List[List[int]], col: int) -> Dict[int, int]:
    """
    For a given column, find every value that historically followed the
    CURRENT last-5-row window, with frequency counts (not just the single
...
def _ranked_candidates(
    ...
    Full ranked candidate list over value_domain for a column, ordered by:
      1. order5 tail-match frequency (desc)
    ...
    resolution.
    """
    tail_freq = _order5_tail_frequencies(rows, col)
...
def range_oso5_next(
    ...
    if len(rows) < 6:
        print("Not enough data for range+oso_order5 analysis (need at least 6 draws).")
        return {}

    print("=" * 50)
    print("RANGE + OSO_ORDER5 - Range-Constrained Prediction")
    print("=" * 50)
    ...
    # ---------------------------------------------------------------
    # Stage 2: oso_order5 candidates per column, constrained to range
    # ---------------------------------------------------------------
    ...
    # ---------------------------------------------------------------
    print("\n" + "=" * 50)
    print("RANGE_OSO5_NEXT - FINAL PREDICTION (with source)")
    print("=" * 50)
    ...
    if run_accuracy_test:
        print("\n" + "=" * 50)
        print("RANGE_OSO5_NEXT_MINUS_ONE: Accuracy Test")
        print("=" * 50)
        from range_oso5_next_minus_one import range_oso5_next_minus_one
        range_oso5_next_minus_one(csv_path)
        print("=" * 50)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 50)
    print("RANGE_OSO5_NEXT: Forward Prediction")
    print("=" * 50)
    range_oso5_next(_csv)
