"""
range_oso3_next.py — Range-Constrained OSO Order-3 Predictor
...
Two-stage prediction:
  ...
  2. OSO_ORDER3 STAGE — for each main column, predict candidates the same
     way oso_next's "order3 fallback" stage does (pattern_fallback(rows, 3)):
     what value followed the current 3-row window for that column,
     historically. Candidates are then ranked (3-row match frequency first,
  ...
  3. RANGE CONSTRAINT — the final pick for each column must fall inside the
     predicted [range_min, range_max]. If the raw order3 pick is in range,
...
The mega number (column 6) is NOT range-constrained (the user's range
definition explicitly excludes the mega number) — it's predicted the same
way oso_next predicts mega: order3-style pattern match on column 7, falling
...
# ---------------------------------------------------------------------------
# OSO order3-stage helpers
# ---------------------------------------------------------------------------

def _order3_tail_frequencies(rows: List[List[int]], col: int) -> Dict[int, int]:
    """
    For a given column, find every value that historically followed the
    CURRENT last-3-row window, with frequency counts (not just the single
    best pick — used to build a ranked candidate list).
    """
    if len(rows) < 4:
        return {}
    tail = tuple(rows[-3:][j][col] for j in range(3))
    freqs: Counter = Counter()
    for i in range(len(rows) - 3):
        key = tuple(rows[i + j][col] for j in range(3))
        if key == tail:
            freqs[rows[i + 3][col]] += 1
    return dict(freqs)
...
def _ranked_candidates(
    ...
    Full ranked candidate list over value_domain for a column, ordered by:
      1. order3 tail-match frequency (desc)
    ...
    resolution.
    """
    tail_freq = _order3_tail_frequencies(rows, col)
...
def range_oso3_next(
    ...
    if len(rows) < 4:
        print("Not enough data for range+oso_order3 analysis (need at least 4 draws).")
        return {}

    print("=" * 50)
    print("RANGE + OSO_ORDER3 - Range-Constrained Prediction")
    print("=" * 50)
    ...
    # ---------------------------------------------------------------
    # Stage 2: oso_order3 candidates per column, constrained to range
    # ---------------------------------------------------------------
    ...
    for col in range(1, 6):
        print(f"\n[Column {col} order3 analysis]")
        raw_pred = pattern_fallback(rows, 3, col_range=[col]).get(col)
    ...
    # ---------------------------------------------------------------
    print("\n" + "=" * 50)
    print("RANGE_OSO3_NEXT - FINAL PREDICTION (with source)")
    print("=" * 50)
    ...
    if run_accuracy_test:
        print("\n" + "=" * 50)
        print("RANGE_OSO3_NEXT_MINUS_ONE: Accuracy Test")
        print("=" * 50)
        from range_oso3_next_minus_one import range_oso3_next_minus_one
        range_oso3_next_minus_one(csv_path)
        print("=" * 50)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 50)
    print("RANGE_OSO3_NEXT: Forward Prediction")
    print("=" * 50)
    range_oso3_next(_csv)
