"""
pattern_next_minus_one.py — Accuracy test for pattern_next.
...
def pattern_next_minus_one(
    csv_path: Path = None,
    recent_window: int = 50,
) -> None:
    """
    Test pattern_next accuracy by predicting the last known draw.

    Writes a temporary CSV without the last row, runs pattern_next on it,
    ...
    temp_path = write_temp_csv(
        csv_path, all_rows[:-1], header, delimiter, suffix="_pattern_temp"
    )

    # Silently predict on the reduced dataset
    old_stdout = sys.stdout
    sys.stdout = io.StringIO()
    predicted = pattern_next(
        temp_path,
...
if __name__ == "__main__":
    ...
    pattern_next_minus_one(_csv, recent_window=_recent)
