...
def order4(csv_path: Path = None, top_n: int = None):
    if csv_path is None:
        csv_path = (
    ...
    # Vertical 4-row patterns
    patterns: List[Tuple[int, int, int, int]] = []
    for col in range(1, 6):
        for i in range(len(rows) - 3):
            patterns.append((rows[i][col], rows[i + 1][col], rows[i + 2][col], rows[i + 3][col]))
                ...
                for col_c in range(1, 6):
                    for col_d in range(1, 6):
                        cross_patterns.append((
                            rows[i][col_a], rows[i + 1][col_b],
                            rows[i + 2][col_c], rows[i + 3][col_d],
                        ))
