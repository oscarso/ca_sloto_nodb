...
def order4(csv_path: Path = None, top_n: int = None) -> Counter:
    if csv_path is None:
        if len(sys.argv) > 1:
    ...
    # Build 4-row patterns vertically: for each column 2-6, take the values from 4 consecutive rows
    ...
    # Build cross-column permutation patterns: all combinations across 4 consecutive rows
    cross_patterns: List[Tuple[int, int, int, int]] = []
    for i in range(len(rows) - 3):
