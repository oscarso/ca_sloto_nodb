...
def oso_next_minus_one(csv_path: Path = None, top_n: int = None) -> None:
    ...
    temp_path = write_temp_csv(Path(csv_path), all_rows[:-1], header, delimiter, suffix="_oso_temp")

    # Silently predict on the reduced data
    ...
    print(f"\n[oso_next] Main numbers accuracy: {correct}/{total} correct ({accuracy:.1f}%)")
